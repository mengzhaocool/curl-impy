...
def copy_new_files(src_dir, patches_dir, file_list):
    ...
    for f in file_list:
        ...
        if src.exists():
            ...
            log(f"  Copied: {f}")
            # Also overwrite libcurl.def with our libcurl-impersonate.def
            # CMakeLists.txt references libcurl.def, not libcurl-impersonate.def
            if f == "libcurl-impersonate.def":
                dst_def = lib_dir / "libcurl.def"
                shutil.copy2(src, dst_def)
                log(f"  Copied: libcurl-impersonate.def -> libcurl.def")
        else:
