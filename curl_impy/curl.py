...
def impersonate_list() -> list[str]:
    ...
    Returns built-in targets compiled into the DLL (queried via
    ``curl_easy_impersonate_list``) plus any targets registered at
    runtime via :func:`impersonate_register`.
    """
    result = lib.curl_easy_impersonate_list()
    if result != ffi.NULL:
        builtin = ffi.string(result).decode("utf-8", errors="replace").split(",")
        builtin = [t.strip() for t in builtin if t.strip()]
    else:
        builtin = []
